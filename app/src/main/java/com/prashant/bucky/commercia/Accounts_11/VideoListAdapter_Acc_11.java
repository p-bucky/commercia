...
public class VideoListAdapter_Acc_11  extends BaseAdapter implements YouTubeThumbnailView.OnInitializedListener {

    private Context mContext;
    private Map<View, YouTubeThumbnailLoader> mLoaders;

    public VideoListAdapter_Acc_11(final Context context) {
        mContext = context;
    ...
    @Override
    public int getCount() {
        return YouTubeContent_Acc_11.ITEMS.size();
    }

    @Override
    public Object getItem(int position) {
        return YouTubeContent_Acc_11.ITEMS.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final YouTubeContent_Acc_11.YouTubeVideo item = YouTubeContent_Acc_11.ITEMS.get(position);

        if (convertView == null) {
            //Create the row
            final LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            convertView = inflater.inflate(R.layout.row_layout, parent, false);
//can I call u I can better understand??
